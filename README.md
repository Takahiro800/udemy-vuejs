...
# mixinについて
- dataとかオプションを複数のコンポーネントで共有したい
- 別のjsファイルに書き出す
- それらをimportすれば良い
- 同じ名前のものが被った場合はそのコンポーネントのものが優先される（直感的）
- ただ、両方ともライフサイクルのcreatedは実行され、順序がmixin → componentの順で発火される

## グローバルに定義する
- 他のもの「ディレクティブ」や「filter」はそれぞれのコンポーネントで利用するかどうかコードに記述することで、使うか使わないかを判断させていたが、mixinは常に引用されるので、基本的にグローバルには定義しない
